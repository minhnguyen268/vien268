...
const LichSuCuoc = ({ ID, TYPE_GAME = "keno1p" }) => {
  ...
  const GridColDef = [
    ...
    { field: "nguoiDung", headerName: t("Nguời dùng"), width: 100 },
    {
      field: "noiDung",
  ...
  const submit = async () => {
    try {
      if (dataQuery?.[0].tinhTrang === "hoanTat") {
        toast.error(t("Phiên đã hoàn tất, không thể thay đổi"));
        return;
      }
      if (!tienCuoc) {
        toast.error(t("Cần nhập số tiền cược"));
        return;
      }
      console.log(data);
      if (
        ...
        toast.error(t("Sửa bị trùng bi và cửa"));
        return;
      }
  ...
  return (
    <>
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle> {t("Chỉnh sửa")} </DialogTitle>
        <DialogContent>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ fontSize: 14, width: "70px" }}> {t("Bi")} </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", margin: "10px 0px" }}>
            <div style={{ fontSize: 14, width: "70px" }}> {t("Loại cược")} </div>
            <select
              value={loaiCuoc}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ fontSize: 14, width: "70px" }}> {t("Số tiền")} </div>
        ...
        <DialogActions>
          <Button onClick={handleClose}> {t("Quay lại")} </Button>
          <Button onClick={submit}> {t("Thay đổi")} </Button>
        </DialogActions>
      </Dialog>
      <Box
        sx={{
