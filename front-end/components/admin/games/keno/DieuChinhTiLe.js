  ...
  const handleClickDieuChinh = async () => {
    try {
      ...
      if (!isValidBetPayoutPercent) {
        toast.error(t("Vui lòng chọn tỉ lệ hợp lệ"));
        return;
      }
        ...
        {!isLoading && (
          <>
            <FormControl fullWidth>
              <Typography> {t("Chọn loại bi")} </Typography>
              <Select
            ...
            <FormControl fullWidth>
              <Typography> {t("Chọn loại cửa")} </Typography>
              <Select
                labelId="select-status"
                id="select-status-option"
                label="Status"
